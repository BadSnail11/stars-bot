...
BTN_SELF = "buy_ton_self"
BTN_GIFT = "buy_ton_gift"
...
def get_router(session_maker: async_sessionmaker) -> Router:
    ...
    async def entry(cb: types.CallbackQuery, state: FSMContext):
        ...
        if not user.username:
            ...
            return
        await state.set_state(BuyTON.choose_target)
        await cb.message.edit_text("Кому покупаем TON?", reply_markup=who_kb(BTN_SELF, BTN_GIFT, BTN_CANCEL))
    ...
    async def choose_self(cb: types.CallbackQuery, state: FSMContext):
        ...
        await state.set_state(BuyTON.enter_qty)
        await cb.message.edit_text("Сколько TON купить? (минимум 1, цело число)", reply_markup=back_nav_kb())
    
    # Подарок
    @router.callback_query(F.data == BTN_GIFT)
    async def choose_gift(cb: types.CallbackQuery, state: FSMContext):
        await state.set_state(BuyTON.enter_recipient)
        await cb.message.edit_text("Введите получателя (например, @username).", reply_markup=cancel_kb(BTN_CANCEL))

    # Получатель текстом
    @router.message(BuyTON.enter_recipient)
    async def get_recipient(m: types.Message, state: FSMContext):
        ...
        await state.set_state(BuyTON.enter_qty)
        await m.answer("Сколько TON купить? (минимум 1, цело число)", reply_markup=cancel_kb(BTN_CANCEL))

    # Количество звёзд
    @router.message(BuyTON.enter_qty)
    async def get_qty(m: types.Message, state: FSMContext):
        try:
            ...
            return
        if qty < 1:
            await m.answer("Минимум 1. Укажите количество ≥ 1.", reply_markup=cancel_kb(BTN_CANCEL))
            return
        
        await state.update_data(qty=qty)
        await state.set_state(BuyTON.choose_payment)
        await m.answer(
    ...
    async def pay_ton(cb: types.CallbackQuery, state: FSMContext):
        ...
        if not qty:
            await cb.message.answer("Не вижу количество. Начните заново: «Купить TON».")
            await state.clear()
            return
